...
export function getSeasonsWithProgress(show) {
  ...
  const needsBackfill = seasons.some(
    (season) => season.episodesWatched === undefined || season.status === undefined
  );
  if (!needsBackfill) {
    return seasons;
  }
  ...
  return seasons.map((season) => {
    ...
    if (withStatus.episodesWatched !== undefined) {
      return withStatus;
    }

    const watched = Math.max(0, Math.min(withStatus.episode_count ?? 0, remaining));
    remaining -= watched;
    return { ...withStatus, episodesWatched: watched };
  });
}
